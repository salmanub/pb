---
layout: 'layouts/service-child.njk'
title: "Expert Witness for Poor Workmanship | Claim Against Contractor"
subtitle: 'We document breaches of contract and defects to substantiate your claim'
hero:
  image: "src/assets/images/reclamacion-mala-ejecucion.jpg"
  alt: "Construction defects identified by expert engineer"
  title: "Poor Workmanship Expert Report"
description: "Is the quality of your construction or renovation project not as agreed in Barcelona? We document breaches and execution defects in an expert report so you can claim against the contractor."
permalink: "/en/expert-witness-reports/poor-workmanship-claim/"
lang: 'en'
translations:
  - lang: "es"
    permalink: "/informes-periciales/reclamacion-mala-ejecucion/"
  - lang: "ca"
    permalink: "/ca/informes-pericials/reclamacio-mala-execucio/"
eleventyNavigation:
  key: "Poor Workmanship Claim"
  parent: "Expert Witness Reports"
  title: "Poor Workmanship"
  notshow: true
  order: 4
serviceTitle: 'claims for poor workmanship'

# Parent Service (for breadcrumb and linking back to pillar)
parentService:
  title: 'Expert Witness Reports'
  url: '/en/expert-witness-reports/'

# Key features in the hero section
keyFeatures:
  - 'Objective comparison of the executed work against the contract and budget'
  - 'Technical documentation of quality breaches in materials and finishes'
  - 'Verification of compliance with regulations (Technical Building Code)'
  - 'Economic quantification of defects for the claim'

# Service Process (for sidebar)
serviceProcess:
  - title: 'Contract and Project Review'
    description: 'We analyze the documentation to establish the basis for the claim'
  - title: 'Detailed On-site Inspection'
    description: 'We photographically document every defect and breach of contract'
  - title: 'Deviation Analysis'
    description: 'We compare what was executed with what was agreed upon and with good practice'
  - title: 'Expert Report for Claim'
    description: 'A report with all the evidence to negotiate or file a lawsuit'

# Service Benefits (for sidebar)
serviceBenefits:
  - 'Turn your dissatisfaction into an irrefutable technical argument'
  - 'Greatly facilitates negotiation with the contractor for remediation'
  - 'Provides your lawyer with the expert evidence needed for a lawsuit'
  - 'Avoid paying for poorly executed work or inferior quality materials'

# Related Problems (optional)
relatedProblems:
  - title: 'Construction Audits'
    description: 'An audit during the execution phase can prevent these problems.'
    url: '/en/construction-audits/'
  - title: 'Structural Pathologies'
    description: 'Poor workmanship can lead to serious structural problems.'
    url: '/en/expert-witness-reports/structural-pathologies/'

# Client Testimonials
testimonials:
  - quote: 'The construction company denied the defects in the finishes. The expert report, with hundreds of photos and references to the quality specifications, was so detailed that they had no choice but to accept and repair everything.'
    author: 'Residential Developer'
    role: 'Case: Defects at new construction handover'
  - quote: "We paid for a waterproofing job that failed. The report proved the poor workmanship and allowed us to successfully claim a refund and the cost of the new repair."
    author: 'Carlos Fernández'
    role: 'Owner of a renovated home'

# CTA Section
ctaTitle: 'Is the result of your project not what you expected?'
ctaDescription: "Don't settle. Document the defects with an expert report and claim what is fair."

# Introduction (shown in hero section)
intro: |
  <p>Investing in a construction or renovation project and receiving a deficient result is one of the most frustrating situations. When the quality does not match what was agreed upon, negotiating with the contractor can be a path full of obstacles.</p>
  
  <p>Our <a href="/en/expert-witness-reports/">expert report service</a> for poor workmanship is designed to give you the tool you need: an objective technical opinion that documents every breach and defect, becoming your best argument to demand a solution.</p>
---

## Dissatisfied with Your Project's Outcome? Don't Settle.

You have invested time, money, and hope into a construction or renovation project, but the result is disappointing: poor finishes, lower quality materials than agreed, solutions that don't work, or outright construction defects. Before getting into an endless dispute, it is essential to have a **solid technical argument**.

An expert report for poor workmanship is an objective opinion that contrasts what was executed with what should have been done according to the contract, the budget, and good construction practice (*lex artis*).

### What Do We Document in a Poor Workmanship Report?

Our job is to "audit" the work performed to give you a solid evidentiary basis. We focus on:

* **Breach of Quality Standards:** We verify if the materials and finishes correspond to the quality specifications or the accepted budget.
* **Execution Defects:** We identify and technically describe poorly executed work (incorrect slopes, defective tiling, deficient waterproofing, etc.).
* **Regulatory Non-Compliance:** We check if the adopted solutions comply with the Technical Building Code (CTE in Spain) and other mandatory regulations.
* **Unexecuted Items:** We verify if you have been charged for work that has not been carried out.

### The Claim Process: Steps to Follow

With our report, the claim process is clearer and more effective:

1.  **Amicable Claim:** The first step is to present the report to the contractor. A professional technical opinion is often enough for the company to agree to remedy the defects.
2.  **Mediation or Arbitration:** If there is no agreement, the report serves as the technical basis for a mediation process.
3.  **Legal Action:** In the worst-case scenario, our report becomes the key expert evidence for a lawsuit, and we will attend court to ratify it before the judge if necessary.

Don't argue without proof. Provide a technical report that speaks for you.