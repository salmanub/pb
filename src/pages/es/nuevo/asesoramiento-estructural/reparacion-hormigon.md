---
layout: 'layouts/service-child.njk'
title: "Reparación de Hormigón Armado | Proyectos Técnicos en Barcelona"
subtitle: 'Diagnosticamos la causa del deterioro y definimos el protocolo de reparación'
description: "Servicio de asesoramiento y elaboración de proyectos para la reparación de hormigón armado en Barcelona. Diagnosticamos la corrosión de armaduras, carbonatación y definimos la solución técnica."
permalink: "/asesoramiento-estructural/reparacion-hormigon/"
lang: 'es'
eleventyNavigation:
  key: "Reparación de Hormigón"
  parent: "Asesoramiento Estructural"
  title: "Reparación de Hormigón"
  notshow: true
  order: 3
serviceTitle: 'reparación de hormigón'

# Parent Service (for breadcrumb and linking back to pillar)
parentService:
  title: 'Asesoramiento Estructural'
  url: '/asesoramiento-estructural/'

# Key features in the hero section
keyFeatures:
  - 'Diagnóstico preciso de la causa del deterioro (carbonatación, cloruros, etc.)'
  - 'Medición de la profundidad de carbonatación y estado de las armaduras'
  - 'Definición de un protocolo de reparación conforme a la norma UNE-EN 1504'
  - 'Especificación de los morteros y sistemas de protección adecuados'

# Service Process (for sidebar)
serviceProcess:
  - title: 'Inspección visual y cata'
    description: 'Evaluamos la extensión del daño y realizamos catas para ver las armaduras'
  - title: 'Análisis y diagnóstico'
    description: 'Realizamos ensayos (test de fenolftaleína) para determinar la causa'
  - title: 'Redacción del protocolo'
    description: 'Detallamos el paso a paso de la reparación: saneado, limpieza, pasivación y aplicación'
  - title: 'Especificación de productos'
    description: 'Seleccionamos los materiales idóneos (puentes de unión, morteros R3/R4, inhibidores)'

# Service Benefits (for sidebar)
serviceBenefits:
  - 'Asegure una reparación duradera que ataca la raíz del problema'
  - 'Evite las "reparaciones estéticas" que ocultan el daño y fallan a corto plazo'
  - 'Cumpla con la normativa europea de reparación de estructuras de hormigón'
  - 'Recupere la seguridad estructural y proteja su inversión a largo plazo'

# Related Problems (optional)
relatedProblems:
  - title: 'Refuerzo con Fibra de Carbono'
    description: 'Una vez reparado el hormigón, a veces es necesario un refuerzo adicional.'
    url: '/asesoramiento-estructural/refuerzo-fibra-carbono/'
  - title: 'Patologías Estructurales'
    description: 'La corrosión de armaduras es una de las patologías estructurales más graves.'
    url: '/informes-periciales/patologias-estructurales/'

# Client Testimonials
testimonials:
  - quote: 'Pensábamos que era solo un problema de pintura en los balcones. Su informe reveló un problema de carbonatación avanzado. El protocolo de reparación que nos prepararon fue la guía perfecta para la constructora.'
    author: 'Comunidad de Propietarios de Sarrià'
    role: 'Caso: Reparación de cantos de forjado en edificio'
  - quote: 'Su diagnóstico fue clave. Identificaron un problema de corrosión por cloruros en nuestro parking y definieron una solución con inhibidores que nos ha dado una solución a largo plazo, no un simple parche.'
    author: 'Jefe de Mantenimiento de Edificio de Oficinas'
    role: 'Caso: Corrosión en pilares de aparcamiento subterráneo'

# CTA Section
ctaTitle: '¿Ve síntomas de deterioro en su hormigón?'
ctaDescription: 'No se limite a tapar el problema. Solicite un diagnóstico técnico para asegurar una reparación correcta y duradera.'

# Introduction (shown in hero section)
intro: |
  <p>El hormigón armado es un material increíblemente duradero, pero no es eterno. La exposición a la intemperie y a agentes agresivos puede iniciar un proceso de deterioro que, si no se trata correctamente, puede comprometer la seguridad de la estructura.</p>
  
  <p>Dentro de nuestro servicio de <a href="/asesoramiento-estructural/">asesoramiento estructural</a>, ofrecemos un diagnóstico experto de las patologías del hormigón y elaboramos el protocolo técnico de reparación que garantiza una intervención eficaz y conforme a la normativa.</p>
---


## Reparar Hormigón: Mucho más que Tapar un Desconchón

Ver cómo el hormigón de una viga, un pilar o un balcón se agrieta y se desprende, dejando a la vista el acero oxidado, es una señal de alarma clara. Una reparación meramente estética, aplicando una capa de mortero sobre la zona dañada, no solo es ineficaz, sino también peligrosa. Sin un diagnóstico y un protocolo adecuados, el proceso de corrosión continuará de forma invisible, y el problema reaparecerá agravado en poco tiempo.

Nuestra labor como ingenieros consultores es definir el "porqué" del deterioro y el "cómo" se debe reparar correctamente para detener el proceso y devolver a la estructura su seguridad y durabilidad.

### Las Causas Ocultas del Deterioro del Hormigón

El hormigón protege al acero de la corrosión gracias a su alta alcalinidad (pH elevado). Cuando esta protección se pierde, el acero se oxida, se expande y rompe el hormigón desde dentro. Las dos causas principales de este fenómeno son:

1.  **Carbonatación:** Es el enemigo más común. El dióxido de carbono ($CO_2$) de la atmósfera penetra en los poros del hormigón y reduce su pH, eliminando la capa de protección natural del acero. Es un proceso lento pero implacable, especialmente en ambientes urbanos.
2.  **Ataque por Cloruros:** Es un enemigo mucho más agresivo. La presencia de iones de cloruro (provenientes de ambientes marinos o de sales de deshielo) destruye la capa protectora del acero incluso con un pH alto, provocando una corrosión muy rápida y localizada.

### Nuestro Protocolo de Actuación: La Norma UNE-EN 1504 como Guía

Una reparación para ser efectiva debe seguir los principios de la normativa europea. Nuestro proyecto técnico detalla cada paso que la empresa ejecutora deberá seguir:

1.  **Saneado del Hormigón:** Eliminación completa del hormigón deteriorado y en mal estado hasta llegar a una base sana y rugosa.
2.  **Limpieza de las Armaduras:** Eliminación de todo el óxido de las barras de acero, normalmente mediante chorreado abrasivo, hasta alcanzar el grado SA 2½. Este paso es **crítico e innegociable**.
3.  **Pasivación de las Armaduras:** Aplicación de un recubrimiento protector (puente de unión) sobre el acero limpio para restaurar su protección frente a la corrosión.
4.  **Recomposición Volumétrica:** Aplicación de un mortero de reparación estructural (tipo R3 o R4) para devolver al elemento su geometría y sección originales.
5.  **Capa de Protección Final:** Aplicación de un revestimiento final (pintura anticarbonatación, hidrofugantes) para proteger la zona reparada y el hormigón adyacente de futuros ataques.

No deje la seguridad de su estructura en manos de una reparación superficial. Un proyecto técnico previo es la garantía de un trabajo bien hecho.