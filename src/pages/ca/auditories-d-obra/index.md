---
layout: "layouts/service-pillar.njk"
title: "Auditoria d'Obres a Barcelona | Control de Costos i Qualitat"
description: "Realitzem auditories d'obra per resoldre disputes en certificacions, controlar sobrecostos i assegurar la qualitat. Obtingui un informe pericial per protegir la seva inversió."
permalink: "/ca/auditories-obra/"
lang: "ca"

translations:
  - lang: es
    permalink: /auditorias-obra/
    title: "Auditoría de Obras en Barcelona | Control de Costes, Calidad y Certificaciones"
    description: "Realizamos auditorías de obra para resolver disputas en certificaciones, controlar sobrecostes y asegurar la calidad. Obtenga un informe pericial para proteger su inversión en Barcelona."
  - lang: en
    permalink: /en/construction-audits/
    title: "Construction Audits in Barcelona | Cost & Quality Control"
    description: "We perform construction audits to resolve certification disputes, control cost overruns, and ensure quality. Get an expert report to protect your investment."
  - lang: fr
    permalink: /fr/audits-de-chantier/
    title: "Audits de Chantier à Barcelone | Contrôle des Coûts et de la Qualité"
    description: "Nous réalisons des audits de chantier pour résoudre les litiges sur les certifications, contrôler les dépassements de coûts et garantir la qualité. Obtenez un rapport d'expertise pour protéger votre investissement."
  - lang: it
    permalink: /it/audit-di-cantiere/
    title: "Audit di Cantiere a Barcellona | Controllo Costi, Qualità e Certificazioni"
    description: "Realizziamo audit di cantiere per risolvere controversie su certificazioni, controllare i costi extra e garantire la qualità. Ottieni una perizia tecnica per proteggere il tuo investimento a Barcellona."

eleventyNavigation:
  key: "Auditories d'Obra"
  title: "Auditories d'Obra"
  notshow: false
  order: 2

hero:
  title: "Auditories Tècniques d'Obra"
  strong: "Protegeixi la seva Inversió i Eviti Conflictes"
  subtitle: "Control expert de qualitat, costos i terminis en projectes de construcció"
  description: "Com a perits enginyers independents, el nostre servei d'auditoria d'obra li proporciona una visió tècnica i objectiva de l'estat real del seu projecte. Detectem desviacions, verifiquem certificacions i documentem la qualitat perquè prengui decisions informades i eviti litigis costosos."
  intro: "Apliquem una metodologia rigorosa per contrastar l'execució en obra amb la documentació del projecte, els contractes i la normativa vigent, lliurant informes pericials clars i defensables."
  image: "src/assets/images/auditoria-de-obra-control-costes-barcelona.jpg"

serviceCluster:
  title: "Els Nostres Àmbits d'Auditoria"
  intro: "Oferim solucions d'auditoria adaptades a cada fase i necessitat del seu projecte:"
  services:
    - title: "Disputes de Certificacions i Amidaments"
      description: "Revisió imparcial de partides, amidaments i preus per resoldre conflictes de pagament i detectar sobrecostos."
      url: "/ca/auditories-obra/disputes-certificacions-obra/"
      icon: "clipboard-list"
      features:
        - "Anàlisi de disputes de pagament"
        - "Verificació d'amidaments"
        - "Detecció de sobrecostos"
    - title: "Auditoria d'Execució i Control de Qualitat"
      description: "Verificació en obra que els materials, solucions constructives i acabats compleixen amb el projecte i la normativa."
      url: "/ca/auditories-obra/control-de-qualitat/"
      icon: "check-square"
      features:
        - "Conformitat de materials"
        - "Supervisió d'execució"
        - "Documentació de defectes"
    - title: "Auditoria de Pre-lliurament i Recepció d'Obra"
      description: "Inspecció final exhaustiva per elaborar una llista de repassos i assegurar un lliurament sense sorpreses."
      url: "/ca/auditories-obra/auditoria-pre-entrega/"
      icon: "file-check"
      features:
        - "Llistes de repassos tècnics"
        - "Proves de servei finals"
        - "Assessorament en actes"
    - title: "Auditoria de Compliment Normatiu i de Projecte" # <-- CONTINGUT ACTUALITZAT
      description: "Verifiquem que l'execució, els materials i les solucions s'ajusten al projecte aprovat i al Codi Tècnic (CTE)."
      url: "/ca/auditories-obra/auditoria-compliment-normatiu-projecte/" # <-- URL ACTUALITZADA
      icon: "file-text"
      features:
        - "Compliment del CTE"
        - "Contrast amb el projecte"
        - "Seguretat jurídica"
    - title: "Inspeccions Tècniques amb Instrumentació"
      description: "Recolzem les nostres auditories amb assajos i equips de mesura per obtenir dades objectives sobre l'estat de l'obra."
      url: "/ca/auditories-obra/inspeccions-tecniques-especialitzades/"
      icon: "search-check"
      features:
        - "Termografia i humitats"
        - "Fissuròmetres i escleròmetre"
        - "Assajos no destructius"

sections:
  title: "Què és una Auditoria Tècnica d'Obra?"
  what_is:
    text: >
      És una revisió independent, tècnica i documental d'una obra per verificar la qualitat d'execució, l'ajust al projecte i la correcció dels amidaments i certificacions. El resultat és un informe pericial amb troballes objectives, evidències traçables i recomanacions clares per a la presa de decisions.
    features:
      - "Inspecció 'in situ' i contrast documental"
      - "Revisió de qualitat, seguretat i normativa"
      - "Verificació de certificacions i partides econòmiques"
      - "Detecció de no conformitats i sobrecostos"
      - "Conclusions tècniques i pla d'accions correctores"

process:
  steps:
    - title: "Anàlisi Preliminar"
      description: "Definim l'abast, la documentació necessària i els objectius de l'auditoria."
    - title: "Inspecció i Presa de Dades"
      description: "Realitzem la visita tècnica, recopilant evidències fotogràfiques i amidaments."
    - title: "Contrast i Avaluació"
      description: "Analitzem les dades davant del projecte, normativa, certificacions i contractes."
    - title: "Informe Pericial d'Auditoria"
      description: "Lliurem el dictamen amb troballes, conclusions i recomanacions defensables."

benefits:
  items:
    - icon: "check-shield"
      title: "Independència i Objectivitat"
      description: "El nostre criteri és purament tècnic i basat en evidències."
    - icon: "law"
      title: "Informes Defensables"
      description: "Amb validesa probatòria per a mediacions, arbitratges o procediments judicials."
    - icon: "target"
      title: "Accions Clares"
      description: "Proposem solucions viables i prioritzades per corregir les desviacions."
    - icon: "trending-down"
      title: "Control de Riscos i Costos"
      description: "Detectem desviacions a temps, evitant sobrecostos i futurs litigis."

testimonials:
  title: "Casos d'Èxit en Auditories"
  items:
    - quote: "L'auditoria va detectar desviacions en amidaments i qualitats. Gràcies a l'informe, vam ajustar les certificacions i vam evitar un sobrecost molt rellevant."
      author: "Constructora Residencial – Barcelona"
      role: "Direcció de Projectes"
    - quote: "L'informe pericial ens va donar la base objectiva que necessitàvem per negociar les correccions amb la promotora abans de la recepció de l'obra."
      author: "Comunitat de Propietaris"
      role: "Administració de Finques"

cta:
  title: "Necessita una visió tècnica i independent de la seva obra?"
  description: "Contacti amb el nostre equip. Analitzarem el seu cas per proposar-li l'enfocament d'auditoria que millor protegeixi els seus interessos."
  button:
    text: "Sol·licitar Avaluació del Cas"
    url: "/ca/contacte/"
---

## Què és una Auditoria Tècnica d'Obra i Quan Hauria de Considerar-la?

Una auditoria tècnica d'obra és un examen exhaustiu i independent d'un projecte de construcció en curs o recentment finalitzat. Pensi en això com una "ITV" d'alta precisió per a la seva inversió. No es basa en opinions, sinó en la **verificació objectiva** de tres pilars fonamentals: que la **qualitat** es correspongui amb la pactada, que els **costos** s'ajustin a la realitat de l'obra executada i que tot el procés compleixi amb la **normativa** vigent.

Hauria de considerar una auditoria si es troba en alguna d'aquestes situacions:
* **Durant l'execució:** Si té dubtes sobre la qualitat dels materials, la correcta execució d'una partida o la validesa de les certificacions que està pagant.
* **Abans de la recepció de l'obra:** És el moment crític per identificar i documentar tots els repassos i defectes pendents abans de donar la conformitat final i alliberar les retencions.
* **Quan sorgeixen conflictes:** Si existeix una disputa amb la constructora sobre amidaments, pagaments o qualitats, un informe d'auditoria és la prova tècnica que pot resoldre el conflicte sense arribar a un litigi.

---

## La Nostra Visió: L'Auditoria com a Eina de Prevenció i Solució

Al nostre despatx, no veiem l'auditoria com un mer informe de "problemes", sinó com una eina estratègica.

* **Com a prevenció:** Una auditoria a temps permet detectar i corregir desviacions abans que es converteixin en problemes greus i costosos. És la forma més eficaç de garantir la qualitat final i evitar sobrecostos inesperats.
* **Com a solució:** Davant d'un conflicte, el nostre informe pericial d'auditoria es converteix en un full de ruta clar i objectiu. Documenta cada troballa, la fonamenta en el contracte o la normativa, i proposa accions correctores, servint com una base sòlida per a la negociació o, si fos necessari, per a una reclamació judicial.

---

## Àmbits Clau de la Nostra Auditoria

Tot i que podem adaptar l'abast a les seves necessitats, les nostres auditories solen centrar-se en les àrees més crítiques d'un projecte:

* **Control de Costos (Certificacions i Amidaments):** Verifiquem que cada euro que paga es correspon amb una obra real i correctament executada. Contrastem els amidaments de les certificacions amb la realitat de l'obra, revisem els preus contradictoris i detectem possibles duplicitats o errors que impacten directament en el seu pressupost.
* **Control de Qualitat (Execució i Materials):** Inspeccionem in situ l'execució de les partides més importants (estructura, impermeabilització, instal·lacions, acabats) per assegurar que s'ajusten a l'especificat en el projecte i compleixen amb la *lex artis* de la construcció. Documentem qualsevol no conformitat per a la seva esmena.
* **Control de Riscos (Recepció d'Obra):** La fase final és la més delicada. Realitzem una inspecció exhaustiva per elaborar una llista de repassos tècnica i completa, assegurant que el projecte es lliura en les condicions pactades i sense vicis ocults que puguin aparèixer en el futur.

---

## Per a Qui és Essencial el Nostre Servei d'Auditoria?

Treballem principalment amb:
* **Promotors i Inversors:** Que necessiten una visió externa i independent per assegurar la rendibilitat i la qualitat de la seva inversió.
* **Comunitats de Propietaris:** Davant la recepció d'obres de rehabilitació o en elements comuns, per garantir que l'execució és correcta.
* **Empreses Constructores:** Que desitgen una auditoria de qualitat externa per validar els seus processos o per mediar en conflictes amb subcontractes.
* **Administracions Públiques:** Per al control i supervisió d'obres licitades.