---
layout: "layouts/service-pillar.njk"
# SEO: Títol ampliat per incloure Obra Civil i Infraestructures.
title: "Assessorament Estructural Barcelona | Edificació i Obra Civil"
# SEO: Descripció que esmenta ambdós sectors i problemes específics.
description: "Assessorament expert en reparació i reforç d'estructures per a edificació i infraestructures (ponts, murs). Diagnostiquem patologies i projectem solucions tècniques."
permalink: "/ca/assessorament-estructural/"
lang: "ca"

translations:
  - lang: es
    permalink: /asesoramiento-estructural/
    title: "Asesoramiento Estructural Barcelona | Edificación y Obra Civil"
    description: "Asesoramiento experto en reparación y refuerzo de estructuras para edificación e infraestructuras (puentes, muros). Diagnosticamos patologías y proyectamos soluciones técnicas."
  - lang: en
    permalink: /en/structural-consulting/
    title: "Structural Consulting in Barcelona | Buildings & Civil Works"
    description: "Expert consulting for structural repair and reinforcement for buildings and infrastructures (bridges, walls). We diagnose pathologies and design technical solutions."
  - lang: fr
    permalink: /fr/conseil-structurel/
    title: "Conseil en Structure à Barcelone | Bâtiment et Génie Civil"
    description: "Conseil expert en réparation et renforcement de structures pour le bâtiment et les infrastructures (ponts, murs). Nous diagnostiquons les pathologies et concevons des solutions techniques."

eleventyNavigation:
  key: "Assessorament Estructural"
  title: "Assessorament Estructural"
  notshow: false
  order: 2

# Hero Section
hero:
  title: "Assessorament Expert en Estructures"
  strong: "Edificació i Enginyeria Civil"
  # MISSATGE: Subtítol que abasta ambdós mons de forma clara.
  subtitle: "Diagnòstic, reparació i reforç d'estructures en edificis i infraestructures"
  description: "Com a enginyers civils experts, oferim un diagnòstic tècnic independent per a qualsevol tipus d'estructura, des de forjats d'habitatges fins a ponts o murs de contenció. Analitzem la causa del problema i projectem la solució de reforç més segura, duradora i eficient."
  intro: "La nostra tasca és proporcionar-vos la certesa tècnica que necessiteu per prendre la millor decisió, optimitzant costos i assegurant la màxima qualitat en la intervenció."
  image: "src/assets/images/asesoramiento-refuerzo-estructural-barcelona.jpg"

# Secció "Per què un diagnòstic independent?" (Aquest missatge és universal i molt potent per a ambdós sectors)
sections:
  title: "Per Què un Assessorament Estructural Independent?"
  what_is:
    text: >
      Contractar un consultor estructural independent abans que una empresa constructora garanteix que la solució proposada respongui a una necessitat tècnica real i no a un interès comercial. Aquest pas previ és la millor inversió per al vostre projecte.
    features:
      - "Garanteix l'elecció de la solució tècnica més eficient, no la més cara"
      - "Proporciona un control de costos real en definir prèviament les actuacions"
      - "Evita reparacions innecessàries o sobredimensionades"
      - "Permet sol·licitar pressupostos a constructores sobre una base tècnica clara"
      - "Actua com una supervisió experta que defensa els vostres interessos"
      - "Assegura el compliment de la normativa específica (EHE-08, CTE, etc.)"

# Service Cluster: Ara inclou serveis d'Edificació I Obra Civil
serviceCluster:
  title: "Els Nostres Serveis de Consultoria Estructural"
  intro: "Analitzem i projectem solucions per als problemes estructurals més complexos en edificació i obra civil:"
  services:
    - title: "Diagnòstic de Patologies (Formigó, Acer)"
      description: "Anàlisi de fissuració, corrosió d'armadures, deformacions, fatiga i altres danys per determinar-ne la causa, l'abast i el nivell de risc."
      url: "/ca/assessorament-estructural/diagnostic-patologies/"
      icon: "search-check"
      features:
        - "Corrosió i carbonatació"
        - "Avaluació de fissures"
        - "Anàlisi de fatiga"
    - title: "Inspecció i Patologia de Ponts i Passarel·les"
      description: "Inspeccions principals i especials de ponts, viaductes i passarel·les per avaluar el seu estat, detectar patologies i planificar actuacions."
      url: "/ca/assessorament-estructural/inspeccio-ponts/"
      icon: "blueprint"
      features:
        - "Inspeccions principals"
        - "Patologia de taulers i piles"
        - "Plans de manteniment"
    - title: "Projectes de Reforç amb Fibra de Carboni"
      description: "Disseny i càlcul de reforços amb composites (CFRP) per a bigues, pilars, forjats i estructures d'obra civil que requereixin augment de capacitat."
      url: "/ca/assessorament-estructural/reforc-fibra-carboni/"
      icon: "tool"
      features:
        - "Aplicacions en ponts i bigues"
        - "Càlcul i dimensionament"
        - "Sistemes SIKA®"
    - title: "Anàlisi de Murs de Contenció i Talussos"
      description: "Avaluació de l'estabilitat de murs (pantalla, escullera), anàlisi d'empentes del terreny, i projectes d'ancoratge o reparació."
      url: "/ca/assessorament-estructural/murs-contencio-talussos/"
      icon: "mountain"
      features:
        - "Estabilitat de murs"
        - "Anàlisi d'empentes"
        - "Solucions d'ancoratge"
    - title: "Reforç i Recalçament de Fonaments"
      description: "Estudi d'assentaments diferencials i problemes al terreny per projectar solucions com micropilots, injeccions o ampliació de sabates."
      url: "/ca/assessorament-estructural/reforc-fonaments/"
      icon: "anchor"
      features:
        - "Anàlisi d'assentaments"
        - "Solucions amb micropilots"
        - "Estabilització del terreny"
    - title: "Projectes de Reparació de Formigó"
      description: "Definim protocols tècnics per a la correcta reparació de formigó danyat segons normativa, tant en edificació com en obra civil."
      url: "/ca/assessorament-estructural/reparacio-formigo/"
      icon: "construction"
      features:
        - "Protocols de sanejament"
        - "Especificació de morters"
        - "Sistemes de protecció"

# Pont cap a Urbenis.com
urbenisBridge:
  title: "Del Diagnòstic a l'Execució: Col·laboració amb Urbenis"
  text: >
    La nostra tasca a **perito.barcelona** és oferir-vos el diagnòstic imparcial i el millor projecte tècnic per a la vostra tranquil·litat i seguretat.
    Per a una **execució especialitzada** amb les màximes garanties, col·laborem estretament amb **urbenis.com**, experts en l'aplicació de sistemes avançats de reforç i reparació estructural.
  cta:
    text: "Conèixer Serveis d'Execució d'Urbenis"
    url: "https://urbenis.com"
    icon: "arrow-right"

# Procés de Treball
process:
  title: "La Nostra Metodologia d'Assessorament"
  steps:
    - title: "Presa de Contacte i Anàlisi Prèvia"
      description: "Estudiem el vostre problema i la documentació existent per entendre la situació."
    - title: "Inspecció Tècnica i Diagnòstic"
      description: "Visitem l'estructura per inspeccionar els danys i prendre les dades necessàries."
    - title: "Proposta Tècnica de Solució"
      description: "Elaborem un informe o projecte amb el diagnòstic i la solució de reparació o reforç més adequada."
    - title: "Supervisió (Opcional)"
      description: "Si ho desitgeu, podem supervisar la correcta execució de les obres per part de la constructora."

# Testimonis (incloent-ne un d'Obra Civil)
testimonials:
  title: "L'Opinió dels Nostres Clients"
  items:
    - quote: "El seu assessorament independent ens va estalviar molts diners. Una altra empresa ens havia proposat una solució molt més cara i invasiva que no era necessària."
      author: "Administrador de Finques, Barcelona"
      role: "Cas: Reforç de forjat en edifici residencial"
    - quote: "El projecte de reforç amb fibra de carboni per al pas inferior va ser impecable. Molt detallat i clar, ens va donar la seguretat tècnica que necessitàvem davant l'Administració."
      author: "Cap d'Obra, Constructora d'Infraestructures"
      role: "Cas: Reforç de biga en obra civil"

# CTA final
cta:
  title: "Teniu dubtes sobre la seguretat d'una estructura?"
  description: "No espereu que el problema s'agreugi. Contacteu amb el nostre equip i obtingueu un diagnòstic tècnic professional, ja sigui per a edificació o obra civil."
  button:
    text: "Sol·licitar Diagnòstic Estructural"
    url: "/ca/contacte/"
---

## La Decisió més Important: Un Diagnòstic Estructural Independent

Quan una estructura presenta símptomes preocupants —una esquerda que creix, una biga que es deforma, la corrosió que avança— la primera decisió és la més crítica: qui ha de diagnosticar el problema? Acudir directament a una empresa que repara i executa pot portar a solucions sobredimensionades o que no ataquen la causa real del problema.

El nostre servei d'**assessorament estructural** es basa en un principi fonamental: **primer, el diagnòstic expert i imparcial; després, la solució.** Com a enginyers civils independents, el nostre únic compromís és amb la seguretat, la durabilitat i l'eficiència econòmica. Analitzem la vostra estructura, determinem l'origen exacte de la patologia i projectem la solució tècnica òptima per al vostre cas concret.

## El Nostre Camp d'Actuació: D'Edificis a Grans Infraestructures

La nostra experiència com a Enginyers Civils ens permet abordar un ampli espectre d'estructures, aplicant els mateixos principis de rigor tècnic en cadascuna d'elles:

* **En Edificació:** Analitzem i projectem solucions per a tot tipus d'immobles: edificis residencials, naus industrials, aparcaments, habitatges unifamiliars, etc. Ens centrem en problemes comuns com el reforç de forjats, la reparació de bigues i pilars, o el recalçament de fonaments.
* **En Obra Civil:** Oferim consultoria experta per a infraestructures. Realitzem inspeccions de ponts i passarel·les, analitzem l'estabilitat de murs de contenció i talussos, i projectem reparacions per a estructures de formigó de gran envergadura sotmeses a condicions exigents.

## Solucions Avançades: Més Enllà de la Reparació Tradicional

L'enginyeria d'estructures avança constantment. No ens limitem a les solucions convencionals; estem especialitzats en el càlcul i la projecció de tècniques avançades que ofereixen millors prestacions amb una menor intervenció. Destaquem especialment en el disseny de **reforços amb composites com la fibra de carboni (CFRP)**, utilitzant sistemes d'empreses líders com SIKA®, per augmentar la capacitat portant de bigues i pilars de forma eficaç i duradora.

## El Pont cap a l'Execució: El Rol d'Urbenis.com

Creiem en l'especialització. La nostra excel·lència resideix en el diagnòstic i l'enginyeria: el "què" i el "com" s'ha de fer. Per a la fase d'execució, el "qui" ho fa, és fonamental comptar amb aplicadors experts.

Per això, un cop definit el projecte de reparació o reforç des de **perito.barcelona**, facilitem el contacte amb **urbenis.com**, la nostra empresa col·laboradora especialitzada en l'execució d'aquests treballs. Aquesta sinergia garanteix un procés sense fissures: el millor projecte d'enginyeria, executat pels millors especialistes.

## Quan Necessiteu Assessorament Estructural Urgent?

No ignoreu els senyals que us dóna el vostre edifici o estructura. Contacteu amb nosaltres si observeu:
* Aparició o creixement d'esquerdes en murs, bigues o forjats.
* Despreniment de trossos de formigó que deixen a la vista les armadures oxidades.
* Deformacions o "fletxes" visibles en bigues o balcons.
* Una ITE (Inspecció Tècnica d'Edificis) amb resultat desfavorable en l'apartat d'estructura.
* La necessitat d'augmentar la capacitat de càrrega d'un forjat per a un nou ús.

Un diagnòstic a temps és la millor garantia de seguretat i us pot estalviar costos significativament més grans en el futur.
