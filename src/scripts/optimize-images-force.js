...
async function processAllImages(forceRegeneration = true) {
  try {
    ...
    try {
      ...
      const jpgPattern = 'assets/images/**/*.{jpg,jpeg}';
      console.log(`Buscando: ${jpgPattern}`);
      jpgImages = await globPromise(jpgPattern, {
        cwd: rootDir,
        ignore: 'assets/images/optimized/**'
      });
      // Convertir a absolutas si es necesario, o ajustar la lógica siguiente
      // Como globPromise usa cwd: rootDir arriba, devoverá relativas.
      // Pero el código original usaba path.join para el pattern, devolviendo absolutas.
      // Estandarizaremos a usar rutas completas para compatibilidad
      jpgImages = jpgImages.map(p => path.join(rootDir, p));

      console.log(`✓ Encontradas ${jpgImages.length} imágenes JPG/JPEG`);

      // Buscar imágenes PNG
      const pngPattern = 'assets/images/**/*.png';
      console.log(`Buscando: ${pngPattern}`);
      pngImages = await globPromise(pngPattern, {
        cwd: rootDir,
        ignore: 'assets/images/optimized/**'
      });
      pngImages = pngImages.map(p => path.join(rootDir, p));

      console.log(`✓ Encontradas ${pngImages.length} imágenes PNG`);

      // Buscar imágenes GIF
      const gifPattern = 'assets/images/**/*.gif';
      console.log(`Buscando: ${gifPattern}`);
      gifImages = await globPromise(gifPattern, {
        cwd: rootDir,
        ignore: 'assets/images/optimized/**'
      });
      gifImages = gifImages.map(p => path.join(rootDir, p));
